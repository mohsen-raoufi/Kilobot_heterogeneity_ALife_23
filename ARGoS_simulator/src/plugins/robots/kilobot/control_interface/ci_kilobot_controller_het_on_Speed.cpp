...
void CCI_KilobotController::Init(TConfigurationNode& t_tree) {
   try {
      ...
      // TO DO: SCIOI noise on Heterogeneity should be updated
      m_cNoiseRange.Set(-0.5, 0.5);
...
void CCI_KilobotController::ControlStep() {
   ...
   // TO DO: SCIOI: ADD HETEROGENEITY HERE!!!!!!!!
   // TO DO: get sure m_pcMotors->SetLinearVelocity() gets double vars in

   double hetFact, hetFact_L, hetFact_R;
   std::string id_str = GetId();
   
   int N = 1;
   float heter_factor = 0;
   // hetFact = (double)(std::stoi( id_str.erase(0,2) ) - 50) * 0.02 * 2.0; // kilo_ID [0 100]
   hetFact = (double)(std::stoi( id_str.erase(0,2) ) - (double)(N/2.0)) * (double)(2.0/N) * heter_factor; // kilo_ID [0 N-1]
   // THIS IS HETEROGENEITY ON SPEED (MAGNITUDE OF VELOCITY)
   hetFact_L = 1.0 + 0.5*hetFact +  m_pcRNG->Uniform(m_cNoiseRange);
   hetFact_R = 1.0 + 0.5*hetFact +  m_pcRNG->Uniform(m_cNoiseRange);

   // std::cout << "het Fact: " << hetFact << std::endl;

   // Notes on the model
   // difference = hetFact_R - hetFact_L = hetFact
   // speed = 0.5*(hetFact_R + hetFact_L) = 1.0

   // hetFact_L = 1 + hetFact;
   // hetFact_R = 1;
   // difference = hetFact_R - hetFact_L = hetFact
   // speed = 0.5*(hetFact_R + hetFact_L) = 1.0 + 0.5*hetFact;

   if(m_pcMotors) {
      m_pcMotors->SetLinearVelocity(6.0 * hetFact_L * m_ptRobotState->right_motor / 255.0,
                                    6.0 * hetFact_R * m_ptRobotState->left_motor / 255.0);
   }
   if(m_pcLED) {
