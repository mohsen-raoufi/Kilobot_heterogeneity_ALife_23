...
namespace argos {
   ...
   static SInt16 ComputeReading(const Real f_distance,
      ...
      if(f_distance < 2.5f) {
         ...
         //fReading *= 1.0f - c_angular_distance / CRadians::PI; // To remove the effect of shadow on the sensor
         fReading *= 1024.0;
   ...
   void CKilobotLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         ...
         // SCIoI
         std::cout << "Hey There! We are using a virtual sensor!" << std::endl;
         // cv::Mat arena_img = cv::imread("arena_pic.png", 0);
         // std::cout << "Arena width: " << arena_img.cols << std::endl;
      }
      catch(CARGoSException& ex) {
   ...
   void CKilobotLightRotZOnlySensor::Update() {
      ...
      double arena_width = 1.0;
      int center_x = 0, center_y = 0; // arena_height = 4, 
      int light_at_center = 1024;
      double x = m_pcEmbodiedEntity->GetOriginAnchor().Position.GetX();
      double y = m_pcEmbodiedEntity->GetOriginAnchor().Position.GetY();
      double dist2Cent = sqrt(x*x + y*y);
      double max_dist = (double)(1.4142*arena_width/2.0);
      // m_pcEmbodiedEntity->GetEmbodiedEntity().GetOriginAnchor().Position.GetX();

      m_nReading = -(double) light_at_center/max_dist * dist2Cent + light_at_center;;

      // std::cout << "x: " << x << ", y: " << y << ", distance 2 Center: " << dist2Cent << ", sensor: " << (double)m_nReading << std::endl;

      if(m_nReading<=0)
         m_nReading = 0;
      // m_nReading = std::min(1024,m_nReading);
      
      //  std::cout << "sensor value: " << m_nReading << std::endl;
